...
| K12 | EFFICIENCY | ravengem 🔨 + MATLAB RAVEN 💡 | 🔨 | **Fast MAFFT (FFT-NS-2) for HMM training** instead of RAVEN's `--auto`, which selects slow iterative refinement (`dvtditr`) on medium/large KOs — observed ~2.5 min/KO (days for a domain) on real KEGG 118. FFT-NS-2 (`--retree 2 --maxiterate 0`) is seconds/KO and ample for profile-HMM building. **PartTree cutover is residue-based** (`parttree_residues`, default 1 M, tunable): MAFFT memory tracks residues (count × length), so a count threshold alone let long-protein KOs (e.g. K00901: 2,788 seqs, 2.55 M residues) OOM under FFT-NS-2 — measured >7 GB with FFT-NS-2 vs **0.69 GB with PartTree** for the same alignment. Back-portable to RAVEN. |
| K13 | EFFICIENCY | ravengem 🔨 | 🔨 | **Per-KO sequence cap** (`max_sequences`, evenly subsampled after CD-HIT) — ravengem's take on RAVEN's `nSequences`, but phylogeny-agnostic (we dropped the phyl-dist matrix, K8). Bounds MAFFT time/memory on the heavily over-represented KOs (prok up to 38,822 seqs); the dropped near-duplicates add little signal to the profile. |
