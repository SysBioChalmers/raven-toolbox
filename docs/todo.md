...
The metabolomics-detected metabolite production-reward block in [`init.ftinit`](../src/raven_python/init/ftinit.py)
...
* **Binary ZIP releases** for BLAST/DIAMOND (Phase 3a). The runtime resolver in
  [`binaries.py`](../src/raven_python/binaries.py) is ready; the registry is empty until ZIPs are
