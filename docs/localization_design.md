...
Design note for porting RAVEN's `predictLocalization.m` to ravengem. **Status: proposal,
...
* the same well-understood `mip_gap` / `time_limit` controls the rest of ravengem uses,
...
The algorithm consumes a `gene × compartment` score table. ravengem ships:
...
1. **How does ravengem mark "uncertain" compartmentalization?** Proposal: a
