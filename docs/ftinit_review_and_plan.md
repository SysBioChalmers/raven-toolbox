...
- `ftINITFillGapsMILP` **hard-errors on glpk-via-COBRA**; "only tested with Gurobi".
  ravengem uses **optlang**, so we are solver-agnostic — but the magic numbers and
...
## 5. What cobra/ravengem already gives us
...
| **4d.6** | **Metabolomics block** (optional `metabolomics_data`): the `mon` production-bonus variables. Defer — the algorithm "is not designed for metabolomics only" and the block is the most intricate, least-evaluated part. | 4d.3 | med |
