...
| 5 | Data integration & analysis (`omics/`, `analysis/`, `comparison/`) | 🟡 started — `reporter_metabolites` + `fseof` + `sampling` (`analysis/`) done; **HPA omics ingestion done** (`parse_hpa`/`parse_hpa_rna`/`hpa_gene_scores`/`rna_gene_scores` in `omics/hpa.py`); dFBA / model comparison pending |
...
| _next_ | Phase 5 omics: `parse_hpa`/`parse_hpa_rna`/`hpa_gene_scores`/`rna_gene_scores` ([src/ravengem/omics/hpa.py](src/ravengem/omics/hpa.py)) — pandas-tidy HPA proteomics + RNA-seq parsers + score adapters that reuse `score_reactions_from_genes` (single GPR walk) |
