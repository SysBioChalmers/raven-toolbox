...
| 7 | Localization (`localization/`) — `predictLocalization` + pluggable predictors (WoLF PSORT, DeepLoc, …); self-contained | 🟢 done — MILP-based `predict_localization` ([localization/predict.py](src/ravengem/localization/predict.py)) with **two new modes vs RAVEN**: partial re-localisation (`reactions_to_relocate=[…]` or auto via `notes['localization']='uncertain'`, rest pinned) and `apply=False` (returns a `LocalizationProposal` diff). Tolerates incomplete models (no silent reaction removal — unplaceable reactions reported, not deleted). Multi-compartment genes with penalty optional. WoLF PSORT + DeepLoc loaders in `scores.py`. See [docs/localization_design.md](docs/localization_design.md) for the critical review + redesign rationale. |
...
| `localization/` | subcellular localization (Phase 7) — `predictLocalization` + pluggable predictors (WoLF PSORT, DeepLoc, …) | 🟢 `predict_localization` MILP + `apply_localization` ([predict.py](src/ravengem/localization/predict.py)); `LocalizationScores` + `load_wolfpsort` + `load_deeploc` ([scores.py](src/ravengem/localization/scores.py)). Critical review of RAVEN's function in [docs/localization_design.md](docs/localization_design.md): partial re-localisation, incomplete-model tolerance, deterministic MILP (not SA), `apply=False` diff mode. |
...
| _next_ | Phase 7 implementation: `localization/scores.py` (`LocalizationScores`, `load_wolfpsort`, `load_deeploc`) + `localization/predict.py` (MILP `predict_localization` + `apply_localization` with `apply=False` proposal mode). 11 tests. |
