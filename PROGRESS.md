...
| 5 | Data integration & analysis (`omics/`, `analysis/`, `comparison/`) | 🟢 in scope done — `reporter_metabolites` + `fseof` + `sampling` (`analysis/`); HPA omics ingestion (`parse_hpa`/`parse_hpa_rna`/`hpa_gene_scores`/`rna_gene_scores` in `omics/hpa.py`); N-model comparison (`compare_models` in `comparison/`); dFBA **cheatsheeted** to the established Python packages (see below) |
| 6 | Visualization (`plotting/`) | ⬜ not started |
| 7 | Localization (`localization/`) — `predictLocalization` + pluggable predictors (WoLF PSORT, DeepLoc, …); self-contained | 🟢 done — MILP-based `predict_localization` ([localization/predict.py](src/ravengem/localization/predict.py)) with **partial re-localisation** (caller passes `reactions_to_relocate` IDs; everything else pinned), **incomplete-model tolerance** (no silent reaction removal), `apply=False` proposal mode, and **multi-compartment-by-default scoring** (primary compartment free, extras pay `multi_compartment_penalty` *plus* their lower predictor score is its own implicit penalty — no hard cutoff). WoLF PSORT + DeepLoc loaders in `scores.py`. `mergeCompartments` + `copyToComps` ported separately as `manipulation/compartments.py` (useful standalone). See [docs/localization_design.md](docs/localization_design.md) for the critical review + redesign rationale. |
...
| `init/init.py` | `run_init` + `InitResult` | `runINIT.m` | ✅ `tests/test_init.py` | The INIT MILP, clean optlang reformulation (split reversibles; `eps·x ≤ v ≤ ub·x` include-indicators; `x_fwd+x_rev≤1` for `no_rev_loops`; per-met reward sinks for `prod_weight`; LP-relaxation `present_mets` test). ⚠️ scale-dependent thresholds (`eps`/`prod_weight`) exposed for tuning. |
| `init/score.py` | `score_reactions_from_genes`, `gene_scores_from_expression` | `scoreComplexModel.m` core | ✅ `tests/test_init_build.py` | GPR scoring (OR→max, AND→min, absent genes omitted, no-gene → -2) + RNA-seq gene scoring `5·ln(level/ref)` clamped (RNA-seq is the common tINIT input; single-cell/HPA are alternative sources). |
| `init/build.py` | `get_init_model` + `InitModelResult` | `getINITModel.m` core | ✅ `tests/test_init_build.py` | tINIT pipeline: scores (gene or reaction) → remove dead ends (regime-consistent with `allow_excretion`) → `run_init`. HPA/single-cell ingestion deferred to Phase 5; auto task-essential + task gap-filling to 4d. |
| `init/` (rest) | ftINIT (4d — critical review) | ⬜ pending |
| `omics/` | HPA omics → gene/reaction scores | 🟢 `parse_hpa`/`parse_hpa_rna` (pandas-tidy DataFrames, replace RAVEN's sparse-matrix/cell-array layout); `hpa_gene_scores`/`rna_gene_scores` adapters reuse `score_reactions_from_genes` (one source of truth for the GPR walk) |
| `localization/` | subcellular localization (Phase 7) — `predictLocalization` + pluggable predictors (WoLF PSORT, DeepLoc, …) | 🟢 `predict_localization` MILP + `apply_localization` ([predict.py](src/ravengem/localization/predict.py)); `LocalizationScores` + `load_wolfpsort` + `load_deeploc` ([scores.py](src/ravengem/localization/scores.py)). Critical review of RAVEN's function in [docs/localization_design.md](docs/localization_design.md): caller-passed relocate set, incomplete-model tolerance, deterministic MILP (not SA), `apply=False` diff mode, multi-compartment-by-default with primary-free / extras-penalised scoring. |
| `manipulation/compartments.py` | `merge_compartments` + `copy_to_compartment` ([RAVEN's `mergeCompartments` + `copyToComps`]; useful for flattening for analysis or building dual-localised pathways, **independently** of localisation prediction). | 🟢 `merge_compartments` returns ``(model, deleted_single_met_reactions, deduplicated_reactions)``; `copy_to_compartment` returns ``(model, new_reaction_ids, new_metabolite_ids)``, idempotent, optional `delete_original=True` for move-not-copy. |
...
| `comparison/` | N-model structural + functional comparison | 🟢 `compare_models` → `ModelComparison` of tidy DataFrames (reactions/metabolites/genes/subsystems presence + Jaccard similarity + optional task pass/fail). Plotting & tSNE/MDS deliberately not ported — one-liners in seaborn/scikit-learn on the DataFrames returned. |
...
| `6a03146` | Phase 5 omics: `parse_hpa`/`parse_hpa_rna`/`hpa_gene_scores`/`rna_gene_scores` ([src/ravengem/omics/hpa.py](src/ravengem/omics/hpa.py)) — pandas-tidy HPA proteomics + RNA-seq parsers + score adapters that reuse `score_reactions_from_genes` (single GPR walk) |
| `c446747` | Phase 5 comparison: `compare_models` ([src/ravengem/comparison/compare.py](src/ravengem/comparison/compare.py)) — N-model structural+functional comparison returning tidy DataFrames (reactions/mets/genes/subsystems presence + Jaccard + optional task pass/fail). Plotting/tSNE deliberately not ported (one-liners with seaborn/sklearn). |
| `057e558` | dFBA **not ported** — established Python implementations exist ([`dfba`](https://pypi.org/project/dfba/), [`reframed`](https://pypi.org/project/reframed/), [`mewpy`](https://pypi.org/project/mewpy/)); re-porting `runDynamicFBA` would duplicate well-maintained prior art (cheatsheet entry in PLAN §2.8). Phase 5 in scope ✅ done. |
| `d3bb6de` | Phase 7 design note: critical review of RAVEN's `predictLocalization` + redesign ([docs/localization_design.md](docs/localization_design.md)). Identifies eleven design choices to rethink (mandatory compartment merge, silent reaction surgery, mono-localised genes, simulated annealing, …) and proposes a partial-update + incomplete-model-tolerant MILP API. |
| `c9b08d8` | Phase 7 implementation: `localization/scores.py` (`LocalizationScores`, `load_wolfpsort`, `load_deeploc`) + `localization/predict.py` (MILP `predict_localization` + `apply_localization` with `apply=False` proposal mode). |
| _next_ | Phase 7 follow-ups: caller-passed relocate set (no `notes['localization']` auto-detect); multi-compartment-by-default with primary-free / extras-penalised scoring (`multi_compartment_penalty`); `manipulation/compartments.py` (`merge_compartments` + `copy_to_compartment`) ported as standalone curation primitives. 23 localisation+compartments tests pass; full suite 460 pass. |
