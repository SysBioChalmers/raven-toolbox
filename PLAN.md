...
| `removeReactions` | **PORT** ✅ | Done ([manipulation/remove.py](src/ravengem/manipulation/remove.py)). Delegates to cobra; adds **separable** orphan cleanup (`remove_orphan_metabolites`/`remove_orphan_genes` independent, vs cobra's coupled `remove_orphans`). |
| `removeMets` | **PORT** ✅ | Done. Delegates to cobra; adds `by_name` to delete a metabolite **across all compartments** at once (cobra is ID-only). Cascade left to cobra (`destructive` / `prune_unused_reactions`). |
...
| RAVEN | Notes |
|---|---|
| `readYAMLmodel`, `writeYAMLmodel` | **DONE** ✅ as `read_yaml_model`/`write_yaml_model` ([io/yaml.py](src/ravengem/io/yaml.py)). **Lens correction:** the `!!omap` "legacy" dialect is **cobra's own YAML format** — `cobra.io.load_yaml_model` already reads a real yeast-GEM/Human-GEM file end to end (4102 rxns), so no separate legacy parser is needed (geckopy rejects it only because it chose the ruamel-*safe* loader). The genuine cobra-absent value, and all this port adds: (1) restore model **identity/provenance** from the `metaData:` block (cobra leaves `id=None`); (2) preserve **RAVEN-only per-entry fields** (`smiles`/`inchis`/`deltaG`/`metFrom`, `confidence_score`/`references`/`rxnFrom`, `protein`) that cobra drops — into `notes` on read, back to top-level on write; (3) preserve foreign sections (GECKO `ec-rxns`/`ec-enzymes`/`gecko_light`) verbatim so ecModels round-trip without being interpreted. |
