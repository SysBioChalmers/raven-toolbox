...
def test_ensure_data_file_downloads_and_caches(served):
    ...
    assert path == cache / "ravengem" / "data" / "kegg-v1" / "ko_reaction.tsv.gz"
...
def test_sha256_mismatch_rejected(served):
    ...
    assert not (cache / "ravengem" / "data" / "kegg-v1" / "rxn_flags.tsv.gz").exists()
...
def test_ensure_kegg_data_fetches_core_set(served):
    ...
    assert out == cache / "ravengem" / "data" / "kegg-v1"
