...
def test_returns_result_shape(model):
    res = random_sampling(model, method="random_objective", n_samples=20, seed=1)
...
def test_samples_are_steady_state(model):
    ...
    res = random_sampling(model, method="random_objective", n_samples=15, seed=2)
...
def test_samples_respect_bounds(model):
    res = random_sampling(model, method="random_objective", n_samples=15, seed=3)
...
def test_seed_is_reproducible(model):
    a = random_sampling(model, method="random_objective", n_samples=10, seed=42).samples
    b = random_sampling(model, method="random_objective", n_samples=10, seed=42).samples
...
def test_good_reactions_reused(model):
    ...
    good = find_good_reactions(model)
    res = random_sampling(model, method="random_objective", n_samples=5, good_reactions=good, seed=0)
    assert res.good_reactions == good


def test_min_flux_runs(model):
    res = random_sampling(model, method="random_objective", n_samples=8, min_flux=True, seed=5)
...
def test_diverse_samples(model):
    ...
    res = random_sampling(model, method="random_objective", n_samples=40, seed=7)
...
def test_rejects_bad_n_samples(model):
    with pytest.raises(ValueError, match="n_samples"):
        random_sampling(model, method="random_objective", n_samples=0)


def test_too_few_good_reactions(model):
    with pytest.raises(ValueError, match="usable reactions"):
        random_sampling(model, method="random_objective", n_samples=5, good_reactions=["sup"], n_objectives=2)
