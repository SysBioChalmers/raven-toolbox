...
def test_default_metabolite_chebi_normalises():
    ...
    # lowercase / mixed-case prefix is canonicalised to uppercase CHEBI: (else it silently misses)
    m.annotation["chebi"] = ["chebi:2", "ChEBI:3"]
    assert default_metabolite_chebi(m) == frozenset({"CHEBI:2", "CHEBI:3"})
    m.annotation["chebi"] = 15589  # a scalar non-string value must not crash
    assert default_metabolite_chebi(m) == frozenset({"CHEBI:15589"})
...
def test_has_functional_parent_is_not_rolled_up(tmp_path):
    """has_functional_parent is a 'derived-from' link, not a subtype: a phosphorylated derivative must
    NOT roll up to its functional parent's specific carrier (g6p must not be cheap for a glucose carrier).
    Only is_a and the protonation/tautomer bridges are traversed."""
    import gzip
    ...
    rel = tmp_path / "rel.tsv.gz"
    with gzip.open(rel, "wt", encoding="utf-8") as fh:
        fh.write("CHEBI:20\thas_functional_parent\tCHEBI:10\n")  # g6p derived-from glucose (NOT a subtype)
        fh.write("CHEBI:20\tis_a\tCHEBI:30\n")                    # g6p is_a phosphate-ester (a real is_a)
    sub = tmp_path / "sub.tsv"
    sub.write_text("x\tCHEBI:10\n", encoding="utf-8")            # carrier substrate = glucose
    onto = SubstrateOntology.load(relations_path=rel, substrates_path=sub)
    assert onto.match(["CHEBI:20"], ["CHEBI:10"]) == 0.0         # derivative NOT rolled up to its parent
    assert onto.match(["CHEBI:20"], ["CHEBI:30"]) == 0.9         # its genuine is_a parent still matches


def test_generic_ancestor_gets_no_sibling_credit(tmp_path):
    """A metabolite that is a generic *ancestor* of a specific curated substrate is not a sibling: a
    broad metabolite must not earn near-full sibling credit for a specific-substrate carrier."""
    import gzip
    ...
    rel = tmp_path / "rel.tsv.gz"
    with gzip.open(rel, "wt", encoding="utf-8") as fh:
        fh.write("CHEBI:3\tis_a\tCHEBI:2\n")   # glucose(3) is_a hexose(2)
        fh.write("CHEBI:2\tis_a\tCHEBI:1\n")
    sub = tmp_path / "sub.tsv"
    sub.write_text("x\tCHEBI:3\n", encoding="utf-8")            # carrier substrate = glucose (specific)
    onto = SubstrateOntology.load(relations_path=rel, substrates_path=sub)
    # metabolite 2 (hexose) is a generic ancestor of substrate 3 (glucose) -> no sibling credit
    assert onto.match(["CHEBI:2"], ["CHEBI:3"], sibling_weight=0.5) == 0.0


def test_compartment_less_metabolite_still_gets_a_cost():
    """Every metabolite base must appear in the returned cost mapping -- even one with no compartment."""
    from raven_toolbox.localization import evidence_aware_transport_cost

    m = cobra.Model("t")
    m.add_metabolites([cobra.Metabolite("a_c", compartment="c"), cobra.Metabolite("nc")])  # nc: no comp
    costs = evidence_aware_transport_cost(m, {}, {})
    assert "a" in costs and "nc" in costs  # the compartment-less base is not silently dropped
