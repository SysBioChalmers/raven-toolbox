...
def test_load_stores_the_missing_sentinel_literally(tmp_path):
    """The ΔG side-car tables write 10000000 for "no valid ΔG", and yeast-GEM's side-car carries it on
    777 of its 4102 reaction rows. load_delta_g_csv does not interpret CSV values, so this is recorded
    like any other matched value -- matching RAVEN's loadDeltaGCSV, which has no sentinel concept at
    all. Callers who need to treat it as absent filter it themselves after loading."""
    m = _toy_model()
    csv = tmp_path / "met_dg.csv"
