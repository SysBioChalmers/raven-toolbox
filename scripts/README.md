...
with ravengem installed (`pip install -e .`). End users never need these.
...
to merge into the runtime registry — `ravengem.data._DATA_REGISTRY` (data) or
`ravengem.binaries._REGISTRY` (binary ZIP bundles). The checksum helper is shared
