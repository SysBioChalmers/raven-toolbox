...
def compare_models(
    ...
    *,
    ...
    ``tasks`` is forwarded to :func:`ravengem.tasks.check_tasks` on each model; pass a
    ...
    if tasks is not None:
        # ravengem.tasks.check_tasks accepts a path or an iterable of Task; preserve task
        # ids for the index. Capture the list once so all models test the same set.
        from ravengem.tasks.tasklist import parse_task_list
